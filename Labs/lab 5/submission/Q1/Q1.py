...
def w_closed_form(X, Y):
    ...
    w_closed =torch.matmul(torch.matmul(torch.linalg.inv(torch.matmul(X.T,X)),X.T),Y)
...
def l1_loss(X, Y, w):
    ...
    loss = torch.sum(torch.abs(torch.matmul(X,w)-Y))/Y.shape[0]
...
def l2_loss(X, Y, w):
    ...
    # Please take care of normalization factor 1/n
    z = Y-torch.matmul(X,w.double())
    loss = float(torch.matmul(z.T,z))/Y.shape[0]
...
def l2_loss_derivative(X, Y, w):
    ...
    derivative = -torch.matmul(X.T, 2*(Y-torch.matmul(X, w.double())))/Y.shape[0]
...
def train_model(X_train, Y_train, X_test, Y_test):
    ...
    w = torch.randn(d, 1)  # initialize weights
