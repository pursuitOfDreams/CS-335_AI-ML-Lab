...
def vectorized_dct(X: np.ndarray) -> np.ndarray:
    ...
    img_dct = np.empty((n1, n2))
    for k1 in range(n1):
      for k2 in range(n2):
        val = 4*np.sum(np.multiply(np.multiply(X,np.cos(v1*k1)),np.cos(v2*k2)))
        img_dct[k1, k2]= val
...
def get_document_ranking(D: np.ndarray, Q: np.ndarray) -> np.ndarray:
    ...
    n,w,k = D.shape
    m,w = Q.shape
    Q1 = np.tile(Q, (k,1,1)) # size will be k x m x w
    D1 = np.transpose(D) # size will be k x w x n

    Y = np.matmul(Q1,D1)
    Y = np.max(Y, axis=2)
    Y = np.sum(Y, axis=1)
    rankings = np.argsort(Y)
    rankings = rankings[::-1]
    return rankings
    # END TODO
...
def compute_state_probability(M: np.ndarray,S_T: int, T: int, S_a: int) -> np.float64:
    ...
    # TODO
    n = M.shape[0]
    initial_state_vec = np.zeros((1,n))
    initial_state_vec[0][S_a] = 1

    result = initial_state_vec
    ...
    print(result[0][S_T])
    return np.round_(result[0][S_T], decimals=4)
    # END TODO
...
def compute_state_probability_vec(M: np.ndarray,S_T: int, T: int, S_a: int) -> np.float64:
    ...
    return np.round_(vec[S_a][S_T], decimals=4)
    # END TODO
